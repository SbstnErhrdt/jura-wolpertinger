...
describe('CI and accessibility release contract', () => {
  ...
  it('keeps release manual, immutable, version-checked, and free of mutable GitHub publishing', async () => {
    ...
    expect(source).toContain('corepack pnpm run release:stage -- --platform windows-x64 --input dist')
    expect(source).toContain('corepack pnpm run release:stage -- --platform linux-x64 --input dist')
