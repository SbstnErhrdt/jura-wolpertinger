...
describe('CI and accessibility release contract', () => {
  ...
  it('keeps release manual, immutable, version-checked, and free of mutable GitHub publishing', async () => {
    ...
    expect(source).toContain('corepack pnpm run release:mac:arm64')
    expect(source).toContain('corepack pnpm run release:mac:x64')
    ...
    expect(source).toContain('corepack pnpm run release:stage --platform mac-arm64 --input release/${{ inputs.version }}')
    expect(source).toContain('corepack pnpm run release:stage --platform mac-x64 --input release/${{ inputs.version }}')
