...
test.describe('Jura Wolpertinger Electron app', () => {
  test('covers writing, focus mode, dark mode, PDF export, submission and correction', async () => {
    ...
    try {
      ...
      await page.locator('.settings-panel', { hasText: 'Nutzer' }).locator('input[placeholder="Name"]').fill('Sebastian')
      await page.locator('.settings-panel', { hasText: 'Nutzer' }).locator('button:has-text("Speichern")').click()
